...
- **v11 (feedback user — "đã hơn nữa")**: liquid phủ toàn bộ — **sheets thành material trong suốt thật** (gen/search/Switch: blur 30-36 + alpha .88 + rim gradient đỉnh, content ghost xuyên qua); **FAB green → tinted glass** (glassProminent); mọi chip/nút nổi trên media (+play, ✕, badge, tag) có blur 12 + inner specular; editor bars rim gradient; paywall tier material; gfoot glass. Lưu ý perf: nhiều surface hơn nhưng sheet/FAB là overlay xuất hiện một-lúc-một — mobile audit (P3.2) vẫn pending.
- **v10 (feedback user — glass chưa đủ "liquid")**: nâng cấp LIQUID GLASS premium — tab bar có **refraction thật** (SVG feDisplacementMap trong backdrop-filter, Chromium; browser khác tự fallback blur nhờ invalid-value), **viền gradient rim** (double background padding-box/border-box), **vệt specular chạy** 7.5s, blur 26-28 + saturate 200% + brightness 1.06, dual inset highlight; chips/nút header cùng recipe rim; header scrolled đậm hơn. Đủ bộ light+dark.
